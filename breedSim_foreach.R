...
breedSimX <- function(nloc,tcorr,ecorr, geno, s1, s2, s3, s4, H, deltaRes, deltaTrial, deltaLoc){
  ...
  breedSim4<- breedSim4[, trial:= str_c(location, "_", test)][
    ,trueBV := mean(bv), by = germplasmName] %>% relocate(trial, .before = bv)
  
  # Calculate nongenetic variance
  
  genVar <- var(breedSim4$bv)
  
  nonGenVar <- (genVar - (genVar*H))/H
  
  # Add trial effect
  
  splitTrial <- split(breedSim4, list(breedSim4$location, breedSim4$test))
  
  trialVar <-  deltaTrial * nonGenVar
  
  for (i in 1:length(splitTrial)) {
    
    splitTrial[[i]][, trialEffect := rep(rnorm(1, mean = 0, sd = sqrt(trialVar)), times = nrow(splitTrial[[i]]))]
    
  }
  
  breedSim5 <-  rbindlist(splitTrial)
  ...
  splitLoc <- split(breedSim5, list(breedSim5$location))
  
  locVar <-  deltaLoc * nonGenVar
  
  for (i in 1:nloc) {
    
    splitLoc[[i]][, locEffect := rep(rnorm(1, 0, sd = sqrt(locVar)), times = nrow(splitLoc[[i]]))]
    
  }
  
  breedSim6 <- rbindlist(splitLoc)
  ...
  splitStudy <- split(breedSim6, list(breedSim6$location))
  
  for (i in 1:nloc) {
    
    splitStudy[[i]][, studyEffect := rep(rnorm(1, 0, sd = sqrt(studyVar)), times = nrow(splitStudy[[i]]))]
    
  }
  
  breedSim7 <- rbindlist(splitStudy)
  ...
  breedSim8 <- breedSim7[, residual := rnorm(nrow(breedSim7), mean = 0, sd = sqrt(resVar))]
...
gsAcc <- function(breedSim){
  ...
  breedSim <- breedSimList[[i]]
  
  asreml.options(workspace = "4gb", pworkspace = "4gb")
...
r2<- foreach(i= 1:length(breedSimList)) %dopar%
  
gsAcc(breedSimList[[i]])

r2Results <- rbindlist(r2)

results<-  rbind(results, r2Results)
...
write_csv(results, "results_gs_acc_h4.csv")
...
for (j in 1:5) {
  ...
  breedSimList <- replicate(10, 
                            breedSim <-  breedSimX(geno = geno, nloc = 5,tcorr = .9,ecorr = .2, H = .4, 
                                                   s1 = 1 ,s2 = 2,s3 = 3,s4 = 4, 
                                                   deltaRes = (1/6), deltaTrial = (1/6), deltaLoc = (2/3)),
                            simplify = FALSE) 
...
write_csv(results, "results_gs_acc_h2.csv")
...
for (j in 1:5) {
  ...
  breedSimList <- replicate(10, 
                            breedSim <-  breedSimX(geno = geno, nloc = 5,tcorr = .9,ecorr = .4, H = .6, 
                                                   s1 = 1 ,s2 = 2,s3 = 3,s4 = 4, 
                                                   deltaRes = (1/6), deltaTrial = (1/6), deltaLoc = (2/3)),
                            simplify = FALSE) 
...
write_csv(results, "results_gs_acc_h6.csv")
...
for (j in 1:5) {
  ...
  breedSimList <- replicate(10, 
                            breedSim <-  breedSimX(geno = geno, nloc = 5,tcorr = .9,ecorr = .4, H = .8, 
                                                   s1 = 1 ,s2 = 2,s3 = 3,s4 = 4, 
                                                   deltaRes = (1/6), deltaTrial = (1/6), deltaLoc = (2/3)),
                            simplify = FALSE) 
...
write_csv(results, "results_gs_acc_h8.csv")
