...
ssh arlyna@130.126.88.31

4bb824LH5bm5

cd field_trial_design
...
breedSimX <- function(nloc,tcorr,ecorr, geno, c,s1,s2,s3,s4, H, deltaRes, deltaTrial, deltaLoc){
...
checks <- rbind(
  rbindlist(replicate(c, breedSim2[is.na(cohort),][,cohort := "S1"], simplify = FALSE)),
  rbindlist(replicate(c, breedSim2[is.na(cohort),][,cohort := "S2"], simplify = FALSE)),
  rbindlist(replicate(c, breedSim2[is.na(cohort),][,cohort := "S3"], simplify = FALSE)),
  rbindlist(replicate(c, breedSim2[is.na(cohort),][,cohort := "S4"], simplify = FALSE))
  )
...
return(breedSim <- breedSim9 %>% relocate(trueBV, .after = bv) %>% 
...
testRRH <- var(breedSim$bv)/(var(breedSim$bv) + var(breedSim$locEffect) + var(breedSim$trialEffect) + var(breedSim$residual))
print(testRRH)

testPrepH <- var(breedSim$bv)/(var(breedSim$bv) + var(breedSim$residual) + var(breedSim$studyEffect))
print(testPrepH)                               
...
breedSim_pRep <- breedSim[design == "unbalanced"] # No Checks, unbalanced replication

blup_pRep <- asreml(fixed = prepPheno ~ germplasmName + cohort,
                          random = ~ location, 
                          residual = ~ idv(units),
                          data = breedSim_pRep, 
...
pred_pRep <- predict.asreml(blup_pRep, classify = "germplasmName",  pworkspace = "8gb")

breedSim_RR <- breedSim[design == "unbalanced" | design == "all" & cohort != "S1" | cohort != "S3"]

blup_RR <- asreml(fixed = prepPheno ~ germplasmName,
                   random = ~ location + trial,
                   residual = ~ idv(units),
                   data = breedSim_RR,
                   na.action = na.method(y = "include", x="include"),
                   workspace="8gb")

pred_RR <- predict.asreml(blup_RR, classify = "germplasmName",  pworkspace = "8gb")

breedSim_RCBD<- breedSim[cohort != "S1" | cohort != "S3" & design == "all"]

blup_RCBD <- asreml(fixed = prepPheno ~ germplasmName,
                 random = ~ location + trial,
                 residual = ~ idv(units),
                 data = breedSim_RCBD,
                 na.action = na.method(y = "include", x="include"),
...
pred_RCBD <- breedSim[cohort != "S1" | cohort != "S3" & design == "all"]