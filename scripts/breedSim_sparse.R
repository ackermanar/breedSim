...
# Predicts GP performance on sparse tested trials 
...
for(i in 1:100){
  ...
  for(j in c(.2,.4,.6,.8)){
    ...
    gsAcc <- function(breedSim){
      ...
      resultsByEntry <- breedSim1Loc %>% filter(GSset == "test" & cohort != "check") %>% 
      ...
      resultsByEntry <- breedSimNoLoc %>% filter(GSset == "test" & cohort != "check") %>% 
        distinct(germplasmName, .keep_all = TRUE) %>%
        left_join(GEBVpred, by = "germplasmName") %>%
        dplyr::select(-c(location, block, bv))
    ...
    write_csv(result, str_c("GEBVoutput/GEBVresults/GEBVresults_", i, "_h", j, "_1-16.csv"))
    saveRDS(errors, str_c("GEBVoutput/GEBVerrors/GEBVerrors_", i, "_h", j, "_1-16.Rdata"))
    
    finalResultH2 <- bind_rows(finalResultH2, result)
    
    write_csv(finalResultH2, str_c("GEBVoutput/GEBVresults/breedSimResult_GEBV_", i, "_1-16.csv"))
    
  }
  
  finalResult <- bind_rows(finalResult, finalResultH2)
  write_csv(finalResult, "GEBVoutput/GEBVresults_final/breedSimResult_GEBV_1-16.csv")
